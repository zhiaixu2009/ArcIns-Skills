...
These prompting principles are for `$arc-imagegen`'s API-first CLI workflow:
...
- [API execution controls](#api-execution-controls)
...
## Transparent images
- Use API generation with a chroma-key background first for simple transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain the native-transparency fallback and ask before switching models.
...
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are CLI/API execution controls for `scripts/image_gen.py`.
- See `references/cli.md` and `references/image-api.md` for these controls.
...
- If a transparent request needs true native transparency, ask before using `gpt-image-1.5` unless the user already explicitly chose it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
