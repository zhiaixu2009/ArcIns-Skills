...
These prompt recipes are for `$arc-imagegen`'s API-first CLI workflow:
...
- If transparent output needs true native transparency, ask before using `gpt-image-1.5` unless the user already explicitly requested `gpt-image-1.5` or native transparency. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
